...
- **Docker Compose** files use the `honeystack` network. Core creates it; honeypot modules declare it as `external: true`.
...
1. `docker-compose.<name>.yml` — service on `honeystack` network (external), logs to `${LOG_DIR}/<name>/`, memory-limited
